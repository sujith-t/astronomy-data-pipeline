...
def plot_line_wavelength_vs_flux(file_path: str, lower_limit, upper_limit, title: str):
...
def fit_line(w, f, center, window=10):
    ...
    try:
        popt, _ = curve_fit(gaussian, x, y, p0=[amp_guess, mu_guess, sigma_guess])
        amp, mu, sigma = popt
...
def find_rest_wavelength(file_path: str):
    hdul = fits.open(file_path)
...
def detect_emission_flux(file_path: str) -> dict:
    rest_wavelength, flux = find_rest_wavelength(file_path)
    ...
    for name, center in detection_lines.items():
        flux_val = fit_line(rest_wavelength, flux_cont_sub, center)
...
def star_formation_rate(file_path: str):
    # read the file
    hdul = fits.open(file_path)

    result = detect_emission_flux(file_path)
...
# remove dust effects
def dust_bias_correction(emission_flux: dict) -> dict:
    k = 2.5
    intrinsic_ratio = 2.86
...
def detect_approximate_iron(file_path: str):
...
def element_abundance_profile(file_path: str):
    ratios = {}

    result = detect_emission_flux(file_path)
    corrected = dust_bias_correction(result)
    ...
    ratios["iron_strength"] = detect_approximate_iron(file_path)
